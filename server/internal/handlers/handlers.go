...
func HandleDNSLookup(c *gin.Context) {
	...
	// ✅ serverKey = DoH key (google / cloudflare / quad9 / opendns)
...
// 	reversedIP := dns.ReverseIP(ip)
// 	total := len(dns.RBLProviders)

// 	checked := 0
// 	listed := 0

// 	summary := models.BlacklistSummary{
...
// 	for _, rbl := range dns.RBLProviders {
// 		query := fmt.Sprintf("%s.%s", reversedIP, rbl.Host)
